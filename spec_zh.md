# MessagePack specification规范

MessagePack is an object serialization specification like JSON.

MessagePack是和JSON类似的对象序列号规范。

MessagePack has two concepts: **type system** and **formats**.

MessagePack有2个概念: **类型系统type system** and **格式化formats**.

Serialization is conversion from application objects into MessagePack formats via MessagePack type system.

序列化是通过MessagePack的类型系统（type system）把应用对象转换成MessagePack格式（formats）。

Deserialization is conversion from MessagePack formats into application objects via MessagePack type system.

反序列化是通过MessagePack的类型系统（type system）把MessagePack格式（formats）转换成应用对象。

    Serialization序列化:
        Application objects应用对象
        -->  MessagePack type system类型系统
        -->  MessagePack formats (byte array)格式（比特数组byte array）

    Deserialization反序列化:
        MessagePack formats (byte array)格式
        -->  MessagePack type system类型系统
        -->  Application objects 应用对象

This document describes the MessagePack type system, MesagePack formats and conversion of them.

本文档阐述了MessagePack类型系统，MessagePack格式和它们之间的转换。

## Table of contents大纲

* MessagePack specification规范
  * [Type system](#types)类型
      * [Limitation](#types-limitation)类型约束
      * [Extension type](#types-extension-type)类型的扩展类型
  * [Formats](#formats)格式
      * [Overview](#formats-overview)格式概览
      * [Notation in diagrams](#formats-notation)格式记法
      * [nil format family](#formats-nil) nil
      * [bool format family](#formats-bool) 布尔
      * [int format family](#formats-int) 整形整形
      * [float format family](#formats-float) 浮点型
      * [str format family](#formats-str) 字符串
      * [bin format family](#formats-bin) 二进制
      * [array format family](#formats-array) 数组
      * [map format family](#formats-map) 映射
      * [ext format family](#formats-ext) 扩展
  * [Serialization: type to format conversion](#serialization序列化)
  * [Deserialization: format to type conversion](#deserialization反序列化)
  * [Future discussion](#future)特性
    * [Profile](#future-profiles特性介绍)
  * [Implementation guidelines实现指南](#impl)
    * [Upgrade MessagePack specification](#impl-upgrade)升级规范
